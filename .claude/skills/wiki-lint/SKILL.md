...
1. 인바운드 `[[ ]]` 링크 수가 **0**인 노드를 후보(candidate)로 모은다. (인바운드 집계에는 `supersedes`/`superseded_by` 엣지도 포함한다 — 이 엣지는 [2-A] 모순에서만 제외할 뿐, orphan 판단에서는 정당한 인바운드로 친다. 즉 다른 결정이 `supersedes`로 가리키는 deprecated 노드는 고립이 아니다.)
