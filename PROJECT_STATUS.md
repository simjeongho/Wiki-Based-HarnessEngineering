...
| **F** | wiki-lint 야간 배치 (LLM) | 모순·orphan·출처누락 — **링크 따라가는 의미적 모순 탐지** | OQ#5(해소) | ✅ **완료** — 스킬(`.claude/skills/wiki-lint/`) + eval 코어 4/4(F1~F4) + e2e verify(라이브 1회, 2커밋). 리포트 `reports/wiki-lint/<날짜>.md`. branch `feat/wiki-lint` |
| **G** | operate agent | 운영 수정 루프 (그래프 진입→원자적 동시 기록) | — | ⏳ 대기 (계획 전) |

**상태 범례:** ✅ 완료 · 🚧 진행중 · 📝 계획중 · ⏳ 대기 · ⛔ 블로킹

**다음 권장 순서:** **F**(`wiki-lint`) 완료 — 의미적 모순·orphan·출처무결성 야간 진단 라이브 검증됨, OQ#5 해소. 이후 **C**(화면정의 진입점, ①을 ②의 특수화로 정렬) / **E**(tdd-implementer 구현 루프, 블로킹 없음) / **G**(operate — wiki-lint 리포트를 소비). B·C는 해당 열린 질문 확정이 선행.
...
| OQ#5 | 야간 wiki-lint 모순 탐지 알고리즘 | F | ✅ **해소** — 링크 그래프 순회 LLM 크롤러(각 노드+1-hop 이웃 판정, supersession 제외). 고도화(임베딩/QMD 후보 선정)는 backlog |
...
- **2026-06-29**
  - ✅ **Sub-project F 완료 — `wiki-lint` 야간 진단 스킬** (`.claude/skills/wiki-lint/SKILL.md`). okf-lint(결정적 구조 게이트) 위에서 기계가 못 잡는 의미·판단 계층을 보는 **비차단 LLM 진단기**. 세 탐지기 — ① **의미적 모순**(링크 그래프 순회, 각 노드+1-hop 이웃 LLM 판정), ② **orphan**(인바운드 0 후보 중 capability 허브·index 진입점 같은 정당 root 제외), ③ **출처 무결성**(`sources[].path` 디스크 존재). 결과는 `reports/wiki-lint/<날짜>.md`에만 기록(wiki/raw/index/log 불변, FA6). 핵심 결정 — FA2(링크 그래프 크롤러), FA3(**supersession 체인은 모순 제외** — 의도된 old↔new 대체), FA4(orphan 정당 root 제외 + commons→wiki 인바운드·index.md 링크 집계), FA7(리포트는 log/와 형제 `reports/`), FA8(eval/verify 2커밋 보존). **OQ#5 해소**(모순 알고리즘=링크 그래프 순회; 고도화=임베딩/QMD 후보 선정, backlog). **검증**: eval 코어 4/4(F1 모순·F2 false-positive 회피·F3 orphan·F4 출처무결성 — fresh executor 블라인드 실행 + 독립 judge) + **e2e verify 라이브 1회**(실제 fixture 그래프 모순1·orphan1·출처누락1, okf-lint OK 선확인, 2커밋 스냅샷→원복). spec `2026-06-29-wiki-lint-design.md`, plan 동명. branch `feat/wiki-lint`. 백로그: F5(클린)·F6(다중 hop)·F7(심각도), 임베딩 후보 선정, cron 야간 배선. **새 production 코드 0 — eval+verify 주도**.
