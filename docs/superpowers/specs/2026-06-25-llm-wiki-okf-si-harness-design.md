...
| D8 | `log/`는 **단일 책임(변경 이력)** 으로 **프로젝트 root**에 위치 (raw·wiki·code 변경 모두 기록, 날짜별) | log가 raw까지 가로지르므로 wiki 밖. |
...
├─ CLAUDE.md           프로젝트용 (@import 공통 규칙 + 프로젝트 특이사항)
├─ raw/                ★원본 산출물·이미지 + PI 확정 화면정의 적재. 불변(에이전트 쓰기금지)
├─ log/                ★날짜별 변경 이력 (raw·wiki·code 전부). 단일 책임
│   ├─ index.md
│   └─ 2026-06-25.md …
├─ wiki/               LLM이 raw를 해석해 작성하는 OKF 번들 (= LLM-Wiki)
│   ├─ index.md            루트 manifest
...
### 예약 파일
- **`wiki/index.md`**: OKF 루트 manifest — 번들 지도 + commons-wiki 링크 + 타입별 목록.
- **`log/<날짜>.md`**: 변경 이력 엔트리 (예: `[add] raw 수집`, `[create] concept`, `[confirm] PI 승인`, `[code] 구현·last_verified 갱신`).
